...
###############################################################################
#                                                                             # 
# Nagios plugin to monitor a single url MD5 sum. In case of mismatch        #
...
# Update 2023: Add curl of the url, download the file (source code) under /tmp#
#   Check the md5 of the file downloaded and compare with --md5		      #
#                                                                             #
###############################################################################

VERSION="1.1"
AUTHOR="(c) 2012 Jack-Benny Persson (jack-benny@cyberinfo.se)"
UPDATE="Ark Systemes(c) 2023 - Ark Operateur"
...
#curl -Lkso /tmp/${file} "${url}"
rand=`echo $RANDOM | md5sum | head -c 5`
fname="${file}_${rand}"
curl -Lkso /tmp/${fname} "${url}"

#Get the current checksum of the file
filesum=`md5sum /tmp/${fname} | awk '{print $1}'`
